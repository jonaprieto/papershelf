...
public func process(job: Job, options: Options, overrideName: String? = nil) -> Item {
    ...
    // A file that will be locked again is not being decrypted, whatever it started as. One
    // that no password opened is passed through as it is rather than being sealed with a
    // new one, which would strand it behind a password it never had.
    ...
    if options.moveOriginals {
        ...
        do {
            if status == .decrypted || status == .encrypted {
                // Both rewrite the document; they differ only in what is written with it.
                // A file being locked again is still rebuilt page by page first, because
                // PDFKit would otherwise carry the source's own encryption over.
                guard let clean = decryptedCopy(of: doc),
                    ...
                    return item(destination, .failed,
                                "could not write the copy; original restored under new name")
                }
            } else {
    ...
    do {
        if status == .decrypted || status == .encrypted {
            let temp = directory.appendingPathComponent(".papershelf-\(UUID().uuidString).pdf")
            scratch = temp
            guard let clean = decryptedCopy(of: doc),
                  clean.write(to: temp, withOptions: writeOptions(options.encryption)) else {
